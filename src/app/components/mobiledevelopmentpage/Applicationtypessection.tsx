...
const apps = [
  ...
  },
  {
    id: 3,
    title: "Hybrid App",
    desc: `A (hybrid app) is a software application that combines elements of both native apps and web applications. Hybrid apps are essentially web apps that have been put in a native app shell. Once they are downloaded from an app store and installed locally, the shell is able to connect to whatever capabilities the mobile platform provides through a browser that's embedded in the app. The browser and its plug-ins run on the back end and are invisible to the end user. Hybrid apps are popular because they allow developers to write code for a mobile app once and still accommodate multiple platforms. Because hybrid apps add an extra layer between the source code and the target platform, SoftWorks develops the app to user-center and progressive while users find it easy and the developers feels it's efficiency.`,
    img: "/assets/Mobileapplications/Applicationtypesassets/hybrid.png",
    color: "#8B5CF6",
    imageFirst: true,
  },
];
      ...
      {apps.map((app) => (
        <div
          ...
          className={`grid grid-cols-1 lg:flex lg:gap-x-6 gap-y-6 ${
            !app.imageFirst ? "lg:flex-row-reverse" : ""
          }`}
        >
          {/* Image */}
          ...
          <div className="flex flex-col justify-center gap-4 lg:w-1/2">
            <AnimateOnView variants={headingVariant}>
              <h3
                className="text-gray-800 font-bold text-xl sm:text-2xl hover:scale-105"
            ...
            <AnimateOnView variants={fadeDown}>
              <p className="text-gray-600 text-sm md:text-base">{app.desc}</p>
            </AnimateOnView>
          </div>
        </div>
      ))}
